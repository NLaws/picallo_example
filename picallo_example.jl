...
function main(x1_0 = 1.0, x2_0 = 1.0, itermax = 40; τ=1/4)
    x1s = zeros(itermax); x2s = zeros(itermax); x2eps = zeros(itermax)
    x1s[1] = x1_0; x2s[1] = x2_0; x2eps[1] = x2_0
    for k = 2:itermax
        x1k = x1s[k-1]; x2k = x2s[k-1]; x2keps = x2eps[k-1]
        x1s[k] = x1_kp1(x1k, x2k; τ=τ)
        x2s[k] = x2_kp1(x1k, x2k; τ=τ)
        x2eps[k] = x2_kp1_eps(x1k, x2keps; τ = τ)
    end
    return x1s, x2s, x2eps
end

x1s, x2s, x2eps = main(0.6,0.6)  # 1,1 does not converge !? nor 0.7,0.7 and higher tenths places
plot(x1s, x2s, seriestype = :scatter, label="PS")
plot!(x1s, x2eps, seriestype = :scatter, label="GD")
savefig("start_from_0p6.png")

# xlims!(-.65, 1.05)
# ylims!(-.45, 1.05)

x1s, x2s, x2eps = main(1,1)  # 1,1 does not converge !? nor 0.7,0.7 and higher tenths places
plot(x1s, x2s, seriestype = :scatter, label="PS")
plot!(x1s, x2eps, seriestype = :scatter, label="GD")
